...
% 	imshow(IM)
% 	hold on
%     imshow(imdilate(WS_lines,strel('disk',1)),cmap);
% 	hold on
    
	%mytry 2 this works
% 	WS_lines = varargin{3};
%     WS_lines = imdilate(WS_lines,strel('disk',1));
%     imshow(imoverlay(IM,WS_lines,'green'))
%     hold on
	...
	for i = 1:n_struct
		...
		for j = 1:n
            ...
            if(COLR{i} == 'c')
                plot(s(j).PerimXYList(:,1),s(j).PerimXYList(:,2),COLR{i},'LineWidth',3)
            else
                plot(s(j).PerimXYList(:,1),s(j).PerimXYList(:,2),COLR{i},'LineWidth',3)
            end
		end
