 function [nucstats,xnuc,ynuc,snuc,w,mask,U,CC_size] = find_nuclei_nuc(I,Ip,lims,xp,yp,scalings,varargin)
...
% for nuc int eleimination
Uo = unroll2(I0,xp,yp,Yhatmax); % unrolled strip of nuc layer
...
Uop = [Uo(:,end-(2*N-1):end),Uo,Uo(:,1:2*N)]; % padding Uo image w periodic bc's
...
stats = regionprops(temp_final_img,Uop,'Centroid','PixelList','PixelIdxList','MeanIntensity');
...
%nuc_rng = prctile(nuc_ints,[50 100]);
% nuc_rng = prctile(nuc_ints,[25 100]);

% histogram(nuc_ints)

nuc_indcs = find(nuc_ints<nuc_rng(1));

% nuc_mean = median(nuc_ints);

% figure, imshow(temp_final_img);
...
%    temp_final_img(stats(nuc_indcs(i)).PixelList) = 0;
...
%     temp_I = gaussFiltDU(imadjust(Ip));
%     imshowcat(gcf,mDU(double(temp_I)),nucstats)
	...
	if exist('filename','var') && ~exist('outfilename','var')
        ...
        outputDir = fullfile('NucImages', 'Nuclear', filenameshort);
