...
%		elements in X. Default, the first seven rows of the "lines"
%		colormap.
%		If this is not specified, but you still want to specify other
%		arguments, put empty brackets -- [] -- in place of this argument.
...
%		If this is not specified, but you still want to specify other
%		arguments, put empty brackets -- [] -- in place of this argument.
%	* "outliersvisible": if logical "true", then the outliers on the boxplot
%		will be plotted in their default state (as the "+" symbol with a
%		centered horizontal coordinate). If false, the outliers will not be
%		plotted at all, and this will only be allowed if "plotdots" is
%		true. Default, true. 
%		If this is not specified, but you still want to specify other
%		arguments, put empty brackets -- [] -- in place of this argument.
...
if nArg >= iArg && ~isempty(varargin{iArg})
    ...
    0.6350    0.0780    0.1840]; % lines colormap
end, iArg = iArg + 1;
if nArg >= iArg && ~isempty(varargin{iArg})
...
if nArg >= iArg && ~isempty(varargin{iArg})
	outliersvisible = varargin{iArg}; else
	outliersvisible = true;
end%, iArg = iArg + 1;
...
%
if ~iscell(X)
	X = {X}; % in the case of making a boxplot of one set of data
end
